...
int generic_statat(struct fd *at, const char *path_raw, struct statbuf *stat,
                   bool follow_links) {
  char path[MAX_PATH];
  int err = path_normalize(
      at, path_raw, path, follow_links ? N_SYMLINK_FOLLOW : N_SYMLINK_NOFOLLOW);
  if (err < 0)
    return err;
...
static dword_t sys_stat_path(fd_t at_f, addr_t path_addr, addr_t statbuf_addr,
                             bool follow_links) {
  int err;
  char path[MAX_PATH];
  if (user_read_string(path_addr, path, sizeof(path)))
    return _EFAULT;
  DEBUG_FPRINTF(stderr, "STAT: at=%d path_addr=0x%llx path=\"%s\" follow=%d\n",
                at_f, (unsigned long long)path_addr, path, follow_links);
  STRACE("stat(at=%d, path_addr=0x%llx, path=\"%s\", statbuf=0x%x, "
         "follow_links=%d)",
         at_f, (unsigned long long)path_addr, path, statbuf_addr, follow_links);
...
dword_t sys_fstatat64(fd_t at, addr_t path_addr, addr_t statbuf_addr,
                      dword_t flags) {
  return sys_stat_path(at, path_addr, statbuf_addr,
...
dword_t sys_statx(fd_t at_f, addr_t path_addr, int_t flags, uint_t mask,
                  addr_t statx_addr) {
  int err;
  char path[MAX_PATH];
  if (user_read_string(path_addr, path, sizeof(path)))
  ...
  STRACE("statx(at=%d, path=\"%s\", flags=%d, mask=%d, statx=0x%x)", at_f, path,
         flags, mask, statx_addr);
  ...
  // for now, ignore the requested mask and just fill in the same fields as stat
  // returns
  struct statx_ statx = {};
