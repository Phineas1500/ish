...
extern void gadget_add16_x8(void);
extern void gadget_add8_x8(void);
extern void gadget_add32_imm(void);
extern void gadget_add16_imm(void);
extern void gadget_add8_imm(void);
...
extern void gadget_sub16_x8(void);
extern void gadget_sub8_x8(void);
extern void gadget_sub32_imm(void);
extern void gadget_sub16_imm(void);
extern void gadget_sub8_imm(void);
...
extern void gadget_and16_imm(void);
extern void gadget_and8_imm(void);
extern void gadget_and64_x8(void);
extern void gadget_and32_x8(void);
extern void gadget_and16_x8(void);
extern void gadget_and8_x8(void);
...
extern void gadget_pcmpgtb(void);
extern void gadget_unpcklpd(void);
extern void gadget_unpckhpd(void);
...
extern void gadget_pcmpgtb_mem(void);
extern void gadget_unpcklpd_mem(void);
extern void gadget_unpckhpd_mem(void);
extern void gadget_punpcklqdq_mem(void);
...
extern void gadget_comiss_xmm_mem(void);
// STMXCSR / LDMXCSR
extern void gadget_stmxcsr(void);
extern void gadget_ldmxcsr(void);
...
extern void gadget_cvtsd2si_reg64(void);  // cvtsd2si r64, xmm (round to nearest)
extern void gadget_cvtsd2si_reg32(void);  // cvtsd2si r32, xmm
extern void gadget_cvtsd2si_mem64(void);  // cvtsd2si r64, m64
extern void gadget_cvtsd2si_mem32(void);  // cvtsd2si r32, m32
extern void gadget_cvtss2si_reg64(void);  // cvtss2si r64, xmm (round to nearest)
extern void gadget_cvtss2si_reg32(void);  // cvtss2si r32, xmm
extern void gadget_cvtss2si_mem64(void);  // cvtss2si r64, m32
extern void gadget_cvtss2si_mem32(void);  // cvtss2si r32, m32
...
extern void gadget_sqrtss_xmm_xmm(void); // sqrtss xmm, xmm
extern void gadget_sqrtss_xmm_mem(void); // sqrtss xmm, m32
// SSE MINSS/MAXSS - Min/Max Scalar Single
extern void gadget_minss_xmm_xmm(void);  // minss xmm, xmm
extern void gadget_minss_xmm_mem(void);  // minss xmm, m32
extern void gadget_maxss_xmm_xmm(void);  // maxss xmm, xmm
extern void gadget_maxss_xmm_mem(void);  // maxss xmm, m32
...
// LOCK RMW memory gadgets
extern void gadget_lock_add64_mem(void);
extern void gadget_lock_add32_mem(void);
extern void gadget_lock_add16_mem(void);
extern void gadget_lock_add8_mem(void);
extern void gadget_lock_sub64_mem(void);
extern void gadget_lock_sub32_mem(void);
extern void gadget_lock_sub16_mem(void);
extern void gadget_lock_sub8_mem(void);
extern void gadget_lock_and64_mem(void);
extern void gadget_lock_and32_mem(void);
extern void gadget_lock_and16_mem(void);
extern void gadget_lock_and8_mem(void);
extern void gadget_lock_or64_mem(void);
extern void gadget_lock_or32_mem(void);
extern void gadget_lock_or16_mem(void);
extern void gadget_lock_or8_mem(void);
extern void gadget_lock_xor64_mem(void);
extern void gadget_lock_xor32_mem(void);
extern void gadget_lock_xor16_mem(void);
extern void gadget_lock_xor8_mem(void);
...
#define fake_ip (state->ip | (1ul << 63))
  ...
  case ZYDIS_MNEMONIC_PREFETCHT2:
    // Cache/spin loop hints - treat as NOP
    break;
  ...
  case ZYDIS_MNEMONIC_LDMXCSR:
    // LDMXCSR m32 - Load MXCSR register from memory (read and discard)
    if (inst.operand_count >= 1 && inst.operands[0].type == arg64_mem) {
      if (!gen_addr(state, &inst.operands[0], &inst)) {
        g(interrupt); GEN(INT_UNDEFINED); GEN(state->orig_ip); GEN(state->orig_ip); return 0;
      }
  ...
  case ZYDIS_MNEMONIC_XOR:
    ...
    if (inst.operand_count >= 2 && is_gpr(inst.operands[0].type)) {
      ...
      if (inst.has_lock) {
        ...
        GEN(load64_gadgets[8]); // _xtmp = imm
        GEN(xor_mem_imm);
        if (size_bits == size64_64) {
          GEN(gadget_lock_xor64_mem);
        } else if (size_bits == size64_32) {
          GEN(gadget_lock_xor32_mem);
        } else if (size_bits == size64_16) {
          GEN(gadget_lock_xor16_mem);
        } else {
          GEN(gadget_lock_xor8_mem);
        }
        GEN(state->orig_ip);

        // Recompute flags from old ^ imm.
        if (size_bits == size64_64) {
          GEN(gadget_xor64_imm);
        } else if (size_bits == size64_32) {
          GEN(gadget_xor32_imm);
      ...
      if (inst.has_lock) {
        ...
        // Recompute flags from old ^ src.
        GEN(gadget_save_xtmp_to_x8); // x8 = old
        GEN(load_src); // _xtmp = src
        if (size_bits == size64_8 &&
            inst.raw_operands[1].type == ZYDIS_OPERAND_TYPE_REGISTER &&
            zydis_is_high_byte_reg(inst.raw_operands[1].reg.value)) {
        ...
        GEN(gadget_swap_xtmp_x8); // _xtmp = old, x8 = src
        if (size_bits == size64_64) {
          GEN(gadget_xor64_x8);
        } else if (size_bits == size64_32) {
          GEN(gadget_xor32_x8);
  ...
  case ZYDIS_MNEMONIC_ADD:
    if (inst.operand_count >= 2 && is_gpr(inst.operands[0].type)) {
      ...
      if (inst.operands[1].type == arg64_imm) {
        ...
        if (is8 || is16) {
          // 8/16-bit add: source in x8, destination in _xtmp.
          bool src_high_byte =
          ...
          GEN(is8 ? gadget_add8_x8 : gadget_add16_x8);
      ...
      if (inst.has_lock) {
        ...
        // Recompute result locally to set flags: old_mem + src.
        GEN(gadget_save_xtmp_to_x8); // x8 = old_mem
        GEN(load_src); // _xtmp = src
        if (add_mem_sz == size64_8 &&
            inst.raw_operands[1].type == ZYDIS_OPERAND_TYPE_REGISTER &&
            zydis_is_high_byte_reg(inst.raw_operands[1].reg.value)) {
        ...
        GEN(gadget_swap_xtmp_x8); // _xtmp = old_mem, x8 = src
        if (add_mem_sz == size64_64) {
          GEN(gadget_add64_x8);
        } else if (add_mem_sz == size64_32) {
          GEN(gadget_add32_x8);
        ...
        if (add_mem_sz == size64_64) {
          ...
          gadget_t load_src = get_load64_reg_gadget(inst.operands[1].type);
          if (load_src)
            GEN(load_src); // _xtmp = source register
          if (src_high_byte) {
            GEN(gadget_lea_lsr64_imm);
            GEN(8);
          }
          GEN(gadget_swap_xtmp_x8); // _xtmp = memory value, x8 = source
          GEN(add_mem_sz == size64_16 ? gadget_add16_x8 : gadget_add8_x8);
        }
      ...
      int64_t add_imm = inst.operands[1].imm;
      if (add_imm_sz == size64_8)
        add_imm &= 0xFF;
      ...
      if (inst.has_lock) {
        ...
        // Recompute result locally for flags: old_mem + imm.
        if (add_imm_sz == size64_64) {
          GEN(gadget_add64_imm);
        } else if (add_imm_sz == size64_32) {
          GEN(gadget_add32_imm);
  ...
  case ZYDIS_MNEMONIC_SUB:
    if (inst.operand_count >= 2 && is_gpr(inst.operands[0].type)) {
      ...
      if (inst.operands[1].type == arg64_imm) {
        ...
        if (is8 || is16) {
          // 8/16-bit sub: source in x8, destination in _xtmp.
          bool src_high_byte =
              is8 && inst.raw_operands[1].type == ZYDIS_OPERAND_TYPE_REGISTER &&
              zydis_is_high_byte_reg(inst.raw_operands[1].reg.value);
          GEN(gadget_save_xtmp_to_x8); // x8 = destination
          ...
          GEN(is8 ? gadget_sub8_x8 : gadget_sub16_x8);
        } else if (inst.operands[1].type >= arg64_rax &&
                   inst.operands[1].type <= arg64_rdi) {
          // SUB reg, reg (rax-rdi)
          if (is64) {
      ...
      int64_t sub_imm = inst.operands[1].imm;
      if (sub_imm_sz == size64_8)
        sub_imm &= 0xFF;
      ...
      if (inst.has_lock) {
        ...
        // Recompute result locally for flags: old_mem - src.
        GEN(gadget_save_xtmp_to_x8); // x8 = old_mem
        GEN(load_src); // _xtmp = src
        if (sub_reg_sz == size64_8 &&
            inst.raw_operands[1].type == ZYDIS_OPERAND_TYPE_REGISTER &&
      ...
      GEN(state->orig_ip);
      // 4. Subtract [mem] - reg with size-correct flag semantics.
      if (sub_reg_sz == size64_64 || sub_reg_sz == size64_32) {
        // sub64_x8/sub32_x8 compute x8 - _xtmp, so swap first.
  ...
  case ZYDIS_MNEMONIC_CBW:
    // Sign-extend AL to AX (66 98) - only modifies bits 15:0 of RAX
    GEN(load64_gadgets[0]); // load rax
    GEN(gadget_cbw);
    GEN(store64_gadgets[0]); // store rax
    break;
    ...
    if (inst.operand_count >= 2 && is_gpr(inst.operands[0].type)) {
      ...
      if (inst.has_lock) {
        ...
        if (and_mem_size == size64_64) {
          GEN(gadget_lock_and64_mem);
        } else if (and_mem_size == size64_32) {
          GEN(gadget_lock_and32_mem);
        } else if (and_mem_size == size64_16) {
          GEN(gadget_lock_and16_mem);
        } else {
          GEN(gadget_lock_and8_mem);
        }
        GEN(state->orig_ip);
        // Recompute flags from old & imm.
        if (and_mem_size == size64_64) {
        ...
        if (is_gpr(inst.operands[1].type)) {
          ...
          if (inst.has_lock) {
            ...
            if (or_reg_mem_size == size64_64) {
              GEN(gadget_lock_or64_mem);
            } else if (or_reg_mem_size == size64_32) {
              GEN(gadget_lock_or32_mem);
            } else if (or_reg_mem_size == size64_16) {
              GEN(gadget_lock_or16_mem);
            } else {
              GEN(gadget_lock_or8_mem);
            }
            GEN(state->orig_ip);
  ...
  case ZYDIS_MNEMONIC_SHR:
    ...
    if (inst.operand_count >= 1 && is_gpr(inst.operands[0].type)) {
      ...
      if (inst.operand_count == 1) {
        if (shr_mem_size == size64_8) {
          GEN(gadget_shr8_imm);
          GEN(1);
        } else if (shr_mem_size == size64_16) {
          GEN(gadget_shr16_imm);
          GEN(1);
        } else {
  ...
  case ZYDIS_MNEMONIC_SHL:
    ...
    if (inst.operand_count >= 1 && is_gpr(inst.operands[0].type)) {
      ...
      if (inst.operand_count == 1) {
        if (shl_mem_size == size64_8) {
          GEN(gadget_shl8_imm);
          GEN(1);
        } else if (shl_mem_size == size64_16) {
          GEN(gadget_shl16_imm);
          GEN(1);
        } else {
  ...
  case ZYDIS_MNEMONIC_SAR:
    ...
    if (inst.operand_count >= 1 && is_gpr(inst.operands[0].type)) {
      ...
      if (inst.operand_count == 1) {
        if (sar_mem_size == size64_8) {
          GEN(gadget_sar8_imm);
          GEN(1);
        } else if (sar_mem_size == size64_16) {
          GEN(gadget_sar16_imm);
          GEN(1);
        } else {
  ...
  case ZYDIS_MNEMONIC_XADD:
    ...
    if (is_mem(inst.operands[0].type) && is_gpr(inst.operands[1].type)) {
      ...
      if (inst.has_lock) {
        ...
        // Atomic mem += src, returning old mem in _xtmp.
        ...
        // Recompute flags from src + old and then write old to source reg.
        GEN(gadget_save_xtmp_to_x8); // x8 = old
        GEN(load_reg); // _xtmp = src (still original register value)
        if (xadd_size == size64_8 &&
            zydis_is_high_byte_reg(inst.raw_operands[1].reg.value)) {
          GEN(gadget_lea_lsr64_imm);
          GEN(8);
        ...
        GEN(gadget_restore_xtmp_from_x8); // _xtmp = old
        gen_store_reg_partial(state, &inst, 1);
      } else {
        GEN(gadget_save_addr);
  ...
  case ZYDIS_MNEMONIC_PUNPCKLQDQ:
    ...
    if (inst.operand_count >= 2 && is_xmm(inst.operands[0].type)) {
      if (is_xmm(inst.operands[1].type)) {
        ...
        // Memory operand - xmm, m128
        if (!gen_addr(state, &inst.operands[1], &inst)) {
          g(interrupt); GEN(INT_UNDEFINED);
          GEN(state->orig_ip); GEN(state->orig_ip); return 0;
        }
        GEN(gadget_punpcklqdq_mem);
        GEN(get_xmm_index(inst.operands[0].type));
        GEN(state->orig_ip);
      } else {
        g(interrupt);
  ...
  case ZYDIS_MNEMONIC_COMISS:
    // UCOMISS/COMISS xmm, xmm/m32 - Compare scalar single (sets EFLAGS)
    if (inst.operand_count >= 2 && is_xmm(inst.operands[0].type)) {
      int src1_xmm = get_xmm_index(inst.operands[0].type);
      if (is_xmm(inst.operands[1].type)) {
        // xmm, xmm
        int src2_xmm = get_xmm_index(inst.operands[1].type);
        GEN(load64_gadgets[8]); GEN(src2_xmm);
        GEN(gadget_ucomiss_xmm_xmm); GEN(src1_xmm);
      } else {
        // xmm, m32
        if (!gen_addr(state, &inst.operands[1], &inst)) {
          g(interrupt);
  ...
  case ZYDIS_MNEMONIC_SQRTSS: {
    // SQRTSS xmm, xmm/m32 - Square root scalar single
    if (inst.operand_count >= 2 && is_xmm(inst.operands[0].type)) {
      int dst_xmm = get_xmm_index(inst.operands[0].type);
      if (is_xmm(inst.operands[1].type)) {
        int src_xmm = get_xmm_index(inst.operands[1].type);
  ...
  case ZYDIS_MNEMONIC_MAXSS: {
    // MINSS/MAXSS xmm, xmm/m32
    if (inst.operand_count >= 2 && is_xmm(inst.operands[0].type)) {
      int dst_xmm = get_xmm_index(inst.operands[0].type);
      bool is_min = (inst.mnemonic == ZYDIS_MNEMONIC_MINSS);
      if (is_xmm(inst.operands[1].type)) {
        int src_xmm = get_xmm_index(inst.operands[1].type);
        GEN(load64_gadgets[8]);
        GEN(src_xmm);
        GEN(is_min ? gadget_minss_xmm_xmm : gadget_maxss_xmm_xmm);
        GEN(dst_xmm);
      } else if (is_mem(inst.operands[1].type)) {
        if (!gen_addr(state, &inst.operands[1], &inst)) {
          g(interrupt); GEN(INT_UNDEFINED);
        ...
        GEN(is_min ? gadget_minss_xmm_mem : gadget_maxss_xmm_mem);
        GEN(dst_xmm);
        GEN(state->orig_ip);
      } else {
        g(interrupt); GEN(INT_UNDEFINED);
  ...
  case ZYDIS_MNEMONIC_CVTSD2SI:
    // CVTSD2SI r32/r64, xmm/m64 - Convert Scalar Double to Signed Integer (round to nearest)
    if (inst.operand_count >= 2) {
      int is_64bit = (inst.operands[0].size == size64_64);
      enum arg64 dst_type = inst.operands[0].type;
      ...
      if (is_xmm(inst.operands[1].type)) {
        // CVTSD2SI reg, xmm
        int src_xmm = get_xmm_index(inst.operands[1].type);
        GEN(is_64bit ? gadget_cvtsd2si_reg64 : gadget_cvtsd2si_reg32);
        GEN(src_xmm);
        if (dst_type >= arg64_r8 && dst_type <= arg64_r15) {
          GEN(store64_r8_r15[dst_type - arg64_r8]);
        } else {
        ...
        GEN(is_64bit ? gadget_cvtsd2si_mem64 : gadget_cvtsd2si_mem32);
        GEN(state->orig_ip);
        if (dst_type >= arg64_r8 && dst_type <= arg64_r15) {
          GEN(store64_r8_r15[dst_type - arg64_r8]);
        } else {
  ...
  case ZYDIS_MNEMONIC_CVTSS2SI:
    // CVTSS2SI r32/r64, xmm/m32 - Convert Scalar Single to Signed Integer (round to nearest)
    if (inst.operand_count >= 2) {
      int is_64bit = (inst.operands[0].size == size64_64);
      enum arg64 dst_type = inst.operands[0].type;
      ...
      if (is_xmm(inst.operands[1].type)) {
        // CVTSS2SI reg, xmm
        int src_xmm = get_xmm_index(inst.operands[1].type);
        GEN(is_64bit ? gadget_cvtss2si_reg64 : gadget_cvtss2si_reg32);
        GEN(src_xmm);
        if (dst_type >= arg64_r8 && dst_type <= arg64_r15) {
          GEN(store64_r8_r15[dst_type - arg64_r8]);
        } else {
        ...
        GEN(is_64bit ? gadget_cvtss2si_mem64 : gadget_cvtss2si_mem32);
        GEN(state->orig_ip);
        if (dst_type >= arg64_r8 && dst_type <= arg64_r15) {
          GEN(store64_r8_r15[dst_type - arg64_r8]);
        } else {
    ...
    if (inst.operand_count >= 2 && is_xmm(inst.operands[0].type)) {
      ...
      if (is_xmm(inst.operands[1].type)) {
        ...
        switch (inst.mnemonic) {
          ...
          case ZYDIS_MNEMONIC_PCMPGTB:    gadget = gadget_pcmpgtb; break;
          case ZYDIS_MNEMONIC_UNPCKLPD:   gadget = gadget_unpcklpd; break;
          case ZYDIS_MNEMONIC_UNPCKHPD:   gadget = gadget_unpckhpd; break;
          case ZYDIS_MNEMONIC_PAND:        gadget = gadget_pand; break;
          case ZYDIS_MNEMONIC_PANDN:       gadget = gadget_pandn; break;
          case ZYDIS_MNEMONIC_ANDNPD:      gadget = gadget_pandn; break;
          case ZYDIS_MNEMONIC_ANDNPS:      gadget = gadget_pandn; break;
        ...
        switch (inst.mnemonic) {
          ...
          case ZYDIS_MNEMONIC_PCMPGTB:    gadget = gadget_pcmpgtb_mem; break;
          case ZYDIS_MNEMONIC_UNPCKLPD:   gadget = gadget_unpcklpd_mem; break;
          case ZYDIS_MNEMONIC_UNPCKHPD:   gadget = gadget_unpckhpd_mem; break;
          case ZYDIS_MNEMONIC_PAND:        gadget = gadget_pand_mem; break;
          case ZYDIS_MNEMONIC_PANDN:       gadget = gadget_pandn_mem; break;
          case ZYDIS_MNEMONIC_ANDNPD:      gadget = gadget_pandn_mem; break;
          case ZYDIS_MNEMONIC_ANDNPS:      gadget = gadget_pandn_mem; break;
  ...
  case ZYDIS_MNEMONIC_PSHUFD:
    ...
    if (inst.operand_count >= 3 && is_xmm(inst.operands[0].type) &&
        inst.operands[2].type == arg64_imm) {
      int dst_xmm = get_xmm_index(inst.operands[0].type);
      uint8_t imm = inst.operands[2].imm & 0xFF;
      if (is_xmm(inst.operands[1].type)) {
        int src_xmm = get_xmm_index(inst.operands[1].type);
        GEN(gadget_pshufd);
        GEN(dst_xmm);
