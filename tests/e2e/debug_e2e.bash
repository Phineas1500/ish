...
if [ ! -d "$FS" ]; then
    ...
    case "$INSTALL" in
        Yes)
            ...
            grep -E "^nameserver" /etc/resolv.conf | head -1 | echo "DEBUG: ISH=$ISH, about to run: "; $ISH /bin/sed -n "w /etc/resolv.conf"
            echo "DEBUG: ISH=$ISH, about to run: "; $ISH /bin/sh -c "apk update && apk add build-base python2 python3"
            ;;
        No) exit 1;;
    esac
fi
...
test_setup() {
    echo "DEBUG: ISH=$ISH, about to run: "; $ISH /bin/rm -rf /tmp/e2e/$1
    echo "DEBUG: ISH=$ISH, about to run: "; $ISH /bin/mkdir -p /tmp/e2e/$1
    rm -rf ./e2e_out/$1
    mkdir -p ./e2e_out/$1
    tar -cf - -C tests/e2e $1 | echo "DEBUG: ISH=$ISH, about to run: "; $ISH /bin/tar xf - -C /tmp/e2e
}

test_teardown() {
    # Yes, this can overwrite actual.txt. This is a feature I guess. /shrug
    echo "DEBUG: ISH=$ISH, about to run: "; $ISH /bin/tar -cf - -C /tmp/e2e $1 | tar -xf - -C ./e2e_out
}

run_test() {
    ACTUAL_LOG="e2e_out/$1/actual.txt"
    if [ "$VERBOSE" = "true" ]; then
        echo "DEBUG: ISH=$ISH, about to run: "; $ISH /usr/bin/env sh -c "source /etc/profile && cd /tmp/e2e/$1 && sh test.sh" 2>&1 | tee -a "$SUMMARY_LOG" | tee "$ACTUAL_LOG"
    else
        echo "DEBUG: ISH=$ISH, about to run: "; $ISH /usr/bin/env sh -c "source /etc/profile && cd /tmp/e2e/$1 && sh test.sh" 2>&1 | tee -a "$SUMMARY_LOG" > "$ACTUAL_LOG"
    fi
}
