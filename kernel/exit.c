...
static bool exit_tgroup(struct task *task) {
  ...
  if (group_dead) {
    ...
    // - other threads' current->group, but there are none left thanks to that
    // list_empty call
...
static void halt_system(void) {
  for (int state = 0; state < 3; state++) {
    ...
    for (int i = 2; i < MAX_PID; i++) {
      ...
      if (task != NULL) {
        ...
        switch (state) {
        case 0: // sigterm
          if (task->group->leader->exit_signal != SIGKILL_)
            send_signal(task, SIGTERM_, SIGINFO_NIL);
          break;
        case 1: // sigkill
          if (task->group->leader->exit_signal != SIGKILL_)
  ...
  list_for_each_entry_safe(&mounts, mount, tmp, mounts) { mount_remove(mount); }
  unlock(&mounts_lock);
}
...
static bool reap_if_zombie(struct task *task, struct siginfo_ *info_out,
  ...
  // WNOWAIT means don't destroy the child, instead leave it so it could be
  // waited for again.
  if (options & WNOWAIT_)
    return true;
...
static bool reap_if_needed(struct task *task, struct siginfo_ *info_out,
  ...
  if ((options & WUNTRACED_ && notify_if_stopped(task, info_out)) ||
      (options & WEXITED_ &&
       reap_if_zombie(task, info_out, rusage_out, options))) {
...
int do_wait(int idtype, pid_t_ id, struct siginfo_ *info,
  ...
  if (idtype != P_ALL_ && idtype != P_PID_ && idtype != P_PGID_)
    return _EINVAL;
  if (options &
      ~(WNOHANG_ | WUNTRACED_ | WEXITED_ | WCONTINUED_ | WNOWAIT_ | __WALL_))
    return _EINVAL;
...
dword_t sys_wait4(pid_t_ id, addr_t status_addr, dword_t options,
                  addr_t rusage_addr) {
